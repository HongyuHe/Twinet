...
func CreateVeth(spec VethSpec) error {
	...
	deleteHostLink(spec.TempA)
	deleteHostLink(spec.TempB)
	...
	if err := installHalf(spec.TempA, spec.A, mtu); err != nil {
		deleteHostLink(spec.TempA)
		deleteHostLink(spec.TempB)
		return err
	}
	if err := installHalf(spec.TempB, spec.B, mtu); err != nil {
		deleteHostLink(spec.TempB)
		_ = deleteEndpoint(spec.A)
		return err
	}
...
func installHalf(tempName string, ep EndpointSpec, mtu int) error {
	...
	if ep.NSPath == "" {
		if err := renameAndConfigure(link, ep, mtu); err != nil {
			return err
		}
		return nil
	}
	...
	return ns.Do(func() error {
		l, err := netlink.LinkByName(tempName)
		if err != nil {
			return fmt.Errorf("find %s after move: %w", tempName, err)
		}
		return renameAndConfigure(l, ep, mtu)
	})
}
...
func renameAndConfigure(link netlink.Link, ep EndpointSpec, mtu int) error {
	// The link must be down to be renamed.
	if err := netlink.LinkSetDown(link); err != nil {
		return fmt.Errorf("set %s down: %w", link.Attrs().Name, err)
	}
	if link.Attrs().Name != ep.Name {
		if err := netlink.LinkSetName(link, ep.Name); err != nil {
	...
	if ep.Shaping != nil {
		if err := ApplyShaping(link, *ep.Shaping); err != nil {
			return err
		}
	}
...
func applyAddrs(link netlink.Link, addrs []string, authoritative bool) error {
	...
	for _, s := range addrs {
		...
		if err := netlink.AddrAdd(link, addr); err != nil && !errors.Is(err, errExists) {
...
func endpointPresent(ep EndpointSpec) (bool, error) {
	check := func() (bool, error) {
		l, err := netlink.LinkByName(ep.Name)
		if err != nil {
			if IsNotFound(err) {
				return false, nil
			}
...
func configureEndpoint(ep EndpointSpec) error {
	apply := func() error {
		...
		if ep.Shaping != nil {
			return ApplyShaping(l, *ep.Shaping)
		}
		return nil
	}
	if ep.NSPath == "" {
